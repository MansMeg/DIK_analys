...
comp_month <- month
comp_year <- "2013"
path <- "/Users/manma97/Dropbox/projekt/DIK_analys" # getwd()
...
for(file in files){
  assign(x=stringr::str_split(file, pattern = "\\.")[[1]][1], 
         read.csv(paste0(path, "/Data/", file)))
}
...
plt <- 
  ggplot(data=utb_bar_plot0, aes(x=utb_grupp, y=antal)) + 
    ...
    ggtitle(expression(atop("A-kassa och aktivitetsstöd", atop("efter utbildningsgrupp"), "")))
